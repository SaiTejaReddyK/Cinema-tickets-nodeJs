...
describe("TicketRequestValidator Adult Boundary Checks", () => {
  ...
  it("is a max adult ticket request, 20 seats and £500", () => {
    var request = [new TicketTypeRequest("ADULT", 20)];
    var response = calculationService.requestCalculation(request);
    expect(response.errors).not.toContain(
      "please request between 1 and 20 tickets in total",
    );
    expect(response.cost).toEqual(500.0);
    expect(response.totalSeats).toEqual(20);
  });

  it("is a valid number of tickets at min, 1 seat and costs £25", () => {
    var request = [new TicketTypeRequest("ADULT", 1)];
    var response = calculationService.requestCalculation(request);
    expect(response.errors).not.toContain(
      "please request between 1 and 20 tickets in total",
    );
    expect(response.cost).toEqual(25.0);
...
describe("validate adult and children", () => {
  ...
  it("should be valid and return one adult and child allocation, costing £40", () => {
    var request = [
      new TicketTypeRequest("ADULT", 1),
      new TicketTypeRequest("CHILD", 1),
    ];
    ...
    expect(response.cost).toEqual(40);
    expect(response.totalSeats).toEqual(2);
  });

  it("should be valid and return 10 adult and 10 child allocation, costing £400", () => {
    var request = [
...
describe("validate adult and infants", () => {
  ...
  it("should be valid and return one adult and no infant allocation, costing £25", () => {
    var request = [
      new TicketTypeRequest("ADULT", 1),
  ...
  it("should be valid and return 10 adult and no infant allocation, costing £250", () => {
    var request = [
      new TicketTypeRequest("ADULT", 10),
    ...
    var response = calculationService.requestCalculation(request);
    expect(response.errors).toBeEmpty();
    expect(response.cost).toEqual(250.0);
...
describe("validate adult, children and infants", () => {
  ...
  it("should be possible to purchase 2 separate adults with one child and infant ticket, 3 seats and £65", () => {
    var request = [
      new TicketTypeRequest("ADULT", 1),
      new TicketTypeRequest("ADULT", 1),
      new TicketTypeRequest("CHILD", 1),
    ...
    var response = calculationService.requestCalculation(request);
    expect(response.errors).toBeEmpty();
    expect(response.cost).toEqual(65.0);
